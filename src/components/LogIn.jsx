...
const handleLogin = (e) => {
  ...
  return (
    ...
    <h2 className='text-white mb-5'> Log In </h2>
        <input onChange={(e) => setUsernameValue(e.target.value)} className={`bg-white border-2 rounded w-72 h-10 border-green-700 ${invalidInput ? 'invalid-input' : ""}
${invalidInput ? 'invalid-input' : ""}`} placeholder='Username' type='text'/>
        <input onChange={(e) => setPasswordValue(e.target.value)} className={`bg-white border-2 rounded w-72 h-10 border-green-700 ${invalidInput ? 'invalid-input' : ""}
${invalidInput ? 'invalid-input' : ""}`} placeholder='Password' type='password'/>
